...
class FirstTemplate(FirstTaskClass):
    ...
    def displayTask(self):
        ...
        print("На координатной прямой отмечены точки A, B, C и D. Каждой точке соответствует одно из чисел в правом столбце. \nУстановите соответствие между указанными точками и числами. ")
        ...
        for i in range(iterations):
            ...
            pencil.text((img_x / iterations * i + optimal_space, img_y / 2 + 20), f"{current_num}", font=font, fill='black', stroke_width=50)
            current_num += 1
        ...
        print(f"Точки\tЧисла\nA\t\t1) log{downDegree(self.log_a)}{self.log_b}\nB\t\t2) {self.frac}\nC\t\t3) " + u"\u221a" + f"{self.root}\nD\t\t4) ({self.reverse_frac}){degree(-1)}")
